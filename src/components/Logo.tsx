 ...
 * - "mark"  : just the circular emblem
 * - "full"  : emblem + wordmark below
 *
 * The emblem references the corn, sheep, cow and Nile waters from
 * the original brand mark, redrawn as clean vectors so it scales
 * crisply at any size and works on any background.
 */
export function Logo({ className, variant = "full" }: Props) {
  return (
    <div className={clsx("inline-flex flex-col items-center gap-2", className)}>
      ...
      >
        {/* Outer ring */}
        <circle cx="100" cy="100" r="92" fill="#FAF7F2" stroke="#0E2A47" strokeWidth="5" />

        {/* Nile waters at the bottom (the "V" confluence echoes the rivers meeting) */}
        <path
        ...
        {/* Center divider — the Y/confluence shape */}
        <path
